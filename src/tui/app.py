...
# -*- coding: utf-8 -*-
"""
Nexus TUI - Terminal User Interface using Rich + prompt_toolkit
Apache 2.0 License
"""
...
def show_banner():
    ...
    console.print(
        Align.center(
            Text(f"AI Coding Agent v{VERSION}  |  Apache 2.0  |  Type /help for commands", style="dim white")
        )
    )
    console.print()
...
def show_help():
    table = Table(title="Nexus Commands", box=box.ROUNDED, border_style="cyan",
...
def show_status():
    table = Table(title="Nexus Status", box=box.ROUNDED, border_style="cyan")
...
def show_docs():
    docs = """
# Nexus AI Coding Agent — Documentation

## Overview
Nexus is a professional CLI AI coding agent that connects to multiple AI providers
(OpenRouter, Groq, Anthropic, OpenAI) to help you write, edit, debug, and manage code.

## Getting Started
1. Run `nexus.py` (or `python nexus.py`)
2. Type `/config` to set your API key and choose a model
3. Start chatting! Ask Nexus to write code, fix bugs, create files, run commands, etc.
...
## Tool System
Nexus automatically calls tools to perform real actions:
...
**Nexus AI Coding Agent** — Version 1.0.0  
**License**: Apache 2.0  
**Effective Date**: 2024

## 1. Acceptance
By using Nexus, you agree to these terms. If you disagree, do not use the software.

## 2. License (Apache 2.0)
Nexus is licensed under the Apache License 2.0. You are free to:
...
- Nexus does NOT collect, store, or transmit your API keys beyond your local machine
- You must comply with each provider's Terms of Service

## 4. Tool Execution (File Operations & Shell Commands)
- Nexus can execute shell commands and modify files on your system
...
Nexus authors are not liable for:
...
## 10. Contact
Report issues at: https://github.com/nexus-agent
"""
...
## Summary
Nexus is a **local application**. It does NOT collect your data, track you, or phone home.
...
- Never logged or sent to any Nexus server (there isn't one)
...
- Nexus does not scan, index, or upload your files without your request

### Usage Data
- **Zero telemetry** — Nexus collects absolutely no usage statistics
- No crash reports are sent automatically
- No analytics of any kind

## Third-Party AI Providers
When you send messages to Nexus:
...
## Children's Privacy
Nexus is not intended for children under 13.

## Changes to This Policy
Updates will be noted in the changelog.

## Contact
Concerns: https://github.com/nexus-agent/issues
"""
...
Copyright 2024 Nexus AI Coding Agent Contributors
...
class NexusApp:
    ...
    def _create_session(self) -> PromptSession:
        history_file = str(config._data.get("history_file",
                           str(Path(str(config.get("workspace", "."))) / ".nexus_history")))
        try:
            hist = FileHistory(os.path.join(str(Path.home()), ".nexus_prompt_history"))
        except Exception:
            hist = None
    ...
    def _handle_command(self, cmd: str) -> bool:
        ...
        if command in ("/exit", "/quit"):
            ...
            fname = arg.strip() or f"nexus_session_{int(time.time())}.json"
            try:
    ...
    def _stream_response(self, user_input: str):
        ...
        console.print()
        console.print(Rule("[bold blue]Nexus[/bold blue]", style="dim blue"))
    ...
    def run(self):
        ...
        if not config.get_api_key():
            show_banner()
            console.print(Panel(
                "[bold yellow]Welcome to Nexus! 🎉[/bold yellow]\n\n"
